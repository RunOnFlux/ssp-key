...
describe('ConstructTx Lib', () => {
  describe('Verifies constructTx', () => {
    ...
    it.skip('should return selectPublicNonce data when value is valid', async () => {
        const res = await selectPublicNonce(rawTxSepolia,"L1JnbAY7VWB6UwfzUa892MN55p1Fnhikpko2jxkVPtAn6DoJVCJE");
        console.log(res)
    });

    it.skip('should return signAndBroadcastEVM data when value is valid', async () => {
        const res = await signAndBroadcastEVM(
            rawTxSepolia, 
            "sepolia", 
